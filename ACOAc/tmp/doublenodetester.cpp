...
bool arePointsEqual(const std::tuple<int, int>& point1, const std::tuple<int, int>& point2) {
    int x1, y1, x2, y2;
...
bool hasDuplicatePoints(const std::vector<std::tuple<int, int>>& points) {
    int size = points.size();
    for (int i = 0; i < size - 1; i++) {
        for (int j = i + 1; j < size; j++) {
            if (arePointsEqual(points[i], points[j])) {
                std::cout << std::get<0>(points[i]) << " " << std::get<1>(points[i]) << std::endl;
                return true;
            }
        }
    }
